...
RAW_DATA = "ABCDEFGHIJKLMNOPQRSTUVWXYZ abcdefghijklmnopqrstuvwxyz 0123456789.,:/%-+?'\"()=*_[]<>;{}↵"
...
def generate_atlas_image(
    ...
    for i, ch in enumerate(chars):
        ...
        # 先给一个默认值，真正写入前会按字形高度做底对齐
        draw_y = y_pos + offset_y
        ...
        try:
            ...
            # 以每个字形实际像素高度做底部对齐
            draw_y = y_pos + (cell_h - mask_img.height) + offset_y
        except Exception:
            # 回退路径同样按文本 bbox 的底边对齐
            text_bbox = draw.textbbox((0, 0), str(ch), font=font)
            draw_y = y_pos + (cell_h - text_bbox[3]) + offset_y
